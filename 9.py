...
rs = requests.get("http://www.163.com")
m = re.findall(r"news\.163\.com/\d.+?<\/a>", str(rs.text), re.M)
...
"""
rs2 = requests.get(UrlNews)
n = re.findall(r"<P style=.TEXT-INDENT: 2em.>(.*?)<\/P>", str(rs2.text), re.M)
for j in n:
    if len(j) != 0:
        j = j.replace("&nbsp", "\n")
        j = j.replace("<STRONG>", "\n_____")
